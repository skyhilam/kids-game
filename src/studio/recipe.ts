...
import { defaultSheet, parseSheet, type SheetSettings } from './animation';
...
export type Recipe = {
  version: 3; sprite: ModelName; seed: number; palette: PaletteName;
...
export function generateRecipe(sprite: ModelName, seed: number, previous?: Recipe, locks: readonly Lockable[] = []): Recipe {
  ...
  const next: Recipe = {
    version: 3, sprite, seed: seed >>> 0, palette, ...paletteColors(sprite, palette),
    ...
    sheet: previous ? { ...previous.sheet } : defaultSheet(),
  };
...
export function initialRecipe(sprite: ModelName): Recipe {
  return { version: 3, sprite, seed: 20260918, palette: 'original', ...paletteColors(sprite, 'original'), variant: 0, detail: 0, size: 256, scale: 90, mirror: false, sheet: defaultSheet() };
}
...
export function parseRecipe(value: unknown): Recipe {
  if (!isObject(value) || ![1, 2, 3].includes(value.version as number) || !owns(models, value.sprite) || !owns(palettes, value.palette)
  ...
  return {
    version: 3, sprite: value.sprite as ModelName, seed: value.seed, palette: value.palette as PaletteName,
    ...
    sheet: value.version === 3 ? parseSheet(value.sheet) : defaultSheet(),
  };
}
...
export function parseDesign(value: unknown): { context: StudioContext; recipe: Recipe } {
  if (!isObject(value) || value.format !== 'picnic-sprite' || ![1, 2, 3].includes(value.version as number)) throw new Error('這不是素材工房的設計檔。');
...
export function exportDesign(context: StudioContext, recipe: Recipe): string {
  return JSON.stringify({ format: 'picnic-sprite', version: 3, style: 'original-storybook', context, recipe,
